package mc.garakrral.geckos.block.feature;
...
public class SimpleRotatedPillarBlock extends RotatedPillarBlock {
    public SimpleRotatedPillarBlock(Properties properties) {
        super(properties);
    }
    ...
    public @Nullable BlockState getToolModifiedState(BlockState state, UseOnContext context,
        ...
        if (context.getItemInHand().getItem() instanceof AxeItem){
            if (state.is(ModBlocks.RED_WOOD_LOG)){
             return ModBlocks.STRIPPED_RED_WOOD_LOG.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
