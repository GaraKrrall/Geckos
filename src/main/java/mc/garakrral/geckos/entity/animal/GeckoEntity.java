...
import mc.garakrral.geckos.client.GeckosClientUtil;
...
public class GeckoEntity extends ShoulderRidingEntity {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(7, new TemptGoal(this, 1.25D, Ingredient.of(ModItems.DEAD_FLY.get()), false));
    ...
    public boolean isFood(ItemStack food) {
        return food.is(ModItems.DEAD_FLY.get());
    }
    ...
    public AgeableMob getBreedOffspring(@NotNull ServerLevel level, @NotNull AgeableMob partner) {
        ...
        if (babyGecko != null) {
            ...
            if (this.isTame()) {
                babyGecko.setOwnerUUID(this.getOwnerUUID());
                babyGecko.setTame(true);
            }
        }
    ...
    public void tick() {
        ...
        if (isServerSide()) {
            ...
            } else if (this.level().isNight() && !this.isInWaterOrBubble() && !this.isSittingGecko() && !this.hasTarget()) {
                sleepCheckCooldown--;
            ...
            } else if (this.level().isDay() && !this.isInWaterOrBubble()) {
                wakeUpCheckCooldown--;
        ...
        if (isClientSide()) {
            if (this.isCarried()) {
                GeckosClientUtil.clientCarryPreview(this);
            }

            this.setupAnimationStates();
        }

        if (shoulderCooldown > 0) {
            shoulderCooldown--;
        }
    }
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(SHOULDER_RIDING, false);
        this.entityData.define(MORNING_SLEEPING, false);
        this.entityData.define(ON_HEAD, false);
        this.entityData.define(CARRIED, false);
    }
    ...
    public void setSleepingGecko(boolean sleeping) {
    ...
    public void setShoulderRiding(boolean riding) {
        this.entityData.set(SHOULDER_RIDING, riding);
    }
    ...
    public void setShoulderCooldown(int cooldown) {
        this.shoulderCooldown = Mth.clamp(cooldown, 0, MAX_SHOULDER_COOLDOWN);
    }
    ...
    public void setCarryDistance(float dist) {
        carryDistance = Mth.clamp(dist, 0.8F, 3.0F);
    }
    ...
    public void setGeckoVariant(GeckoVariants variant) {
    ...
    public SpawnGroupData finalizeSpawn(@NotNull ServerLevelAccessor level,
        ...
        GeckoVariants variant = Util.getRandom(GeckoVariants.values(), this.random);
    ...
    public boolean onShoulder() {
        return this.isShoulderRiding();
    }

    public void setShoulder(boolean onShoulder) {
        this.setShoulderRiding(onShoulder);
    }

    public void setSleeping(boolean sleeping) {
        this.setSleepingGecko(sleeping);
    }
